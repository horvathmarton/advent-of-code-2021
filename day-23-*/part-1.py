...
def roomsComplete(data):
    return (
        isRoomComplete(data, 3)
        and isRoomComplete(data, 5)
        and isRoomComplete(data, 7)
        and isRoomComplete(data, 9)
    )
...
def canMove(data, x, y):
    return (
        not isInOwnRoom(data, x, y) or isBlockingRoom(data, x, y)
    ) and not isBlockedInRoom(data, x, y)
    ...
    newData = (
        *(
            (
                *(
                    (
                        (field(d, a, b), field(d, x, y))[a == i and b == j],
                        field(d, i, j),
                    )[a == x and b == y]
                    for a in range(len(d[b]))
                ),
            )
