...
def increase_cell(value: int) -> int:
...
def read_file(name: str) -> np.array:
    with open(name) as f:
        ...
        for line in f.read().split("\n"):
            base_grid.append([int(num) for num in list(line)])

        base_grid = np.array(base_grid)

        grid = np.array([])
        prev = base_grid
        operation = np.vectorize(increase_cell)
        for _ in range(5):
            row = prev

            for _ in range(4):
                prev = operation(prev)
                row = np.hstack((row, prev))

            grid = np.vstack((grid, row)) if grid.size else row
